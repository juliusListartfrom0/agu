#!/usr/bin/env python3
"""Screen local-view DINOv2 features with event-grouped MUVS folds."""
...
def main() -> int:
    ...
    if not all(isinstance(payload, dict) for payload in embeddings):
        raise ValueError("expected MUVS local visual JSON objects")
    artifact = screen_muvs_local_visual(embeddings)
