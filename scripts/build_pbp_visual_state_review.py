#!/usr/bin/env python3
"""Build label-hidden raw-frame sheets for offline PBP visual-state review."""
...
SHEET_SCHEMA = "agu.pbp-visual-state-review-sheets.v1"
DECISION_TEMPLATE_SCHEMA = "agu.pbp-visual-state-codex-decisions.v1"
...
def build_review_package(
    *,
    ...
    if panel_width < 128 or rows_per_sheet < 1:
        raise ValueError("invalid visual-state review rendering settings")
    embeddings = verify_anchor_state_embedding_artifact(
        _read_json(embedding_artifact_path)
    )
    additional_blind = (
    ...
    plan = build_visual_state_review_plan(
        embeddings,
        additional_sealed_blind_video_sha256s=additional_blind,
    )
    return _render_review_plan_package(
        plan=plan,
...
def render_review_plan_package(
    *,
    plan_path: Path,
    video_paths: list[Path],
    output_dir: Path,
...
def _render_review_plan_package(
    *,
    plan: dict[str, Any],
    video_paths: list[Path],
    output_dir: Path,
    ...
    if len(videos) != len(video_paths):
        raise ValueError("visual-state review videos must have unique hashes")
    expected_sources = set(plan["source_video_sha256s"])
    if set(videos) != expected_sources:
        raise ValueError("visual-state review videos must exactly match the plan")
    ...
    for source_index, source_sha in enumerate(
        ...
        if not capture.isOpened():
            raise ValueError(
                f"cannot open visual-state review video: {videos[source_sha].name}"
            )
        try:
            examples = examples_by_source[source_sha]
            for batch_index in range(0, len(examples), rows_per_sheet):
                batch = examples[batch_index : batch_index + rows_per_sheet]
                rows = [
                    _render_example_row(
                        capture,
                        example,
                        offsets_seconds=plan["anchor_offsets_seconds"],
                        panel_width=panel_width,
                    )
                ...
                sheet_name = (
                    ...
                    f"sheet-{batch_index // rows_per_sheet + 1:02d}.jpg"
                )
                ...
                if not cv2.imwrite(
                    ...
                    raise RuntimeError(
                        f"failed to write visual-state sheet: {sheet_name}"
                    )
                records.append(
                    {
                        ...
                        "review_ids": [
                            str(example["review_id"]) for example in batch
                        ],
                    }
                )
        finally:
    ...
    _write_decision_template(output_dir, plan)
    return manifest


def _render_example_row(
    ...
    *,
    ...
    if len(frame_indexes) != len(offsets_seconds):
        raise ValueError("review frames do not match the offset protocol")
    panels = []
    for position, (frame_index, offset) in enumerate(
        zip(frame_indexes, offsets_seconds, strict=True)
    ):
        capture.set(cv2.CAP_PROP_POS_FRAMES, frame_index)
        ok, frame = capture.read()
        if not ok or frame is None:
            raise ValueError(f"cannot decode visual-state frame {frame_index}")
        panel = _letterbox(frame, width=panel_width)
        cv2.rectangle(panel, (0, 0), (panel_width, 32), (0, 0, 0), -1)
        label = f"{offset:+.0f}s  frame {frame_index}"
        if position == 0:
            label = f"{example['review_id']}  {label}"
        cv2.putText(
            panel,
            label,
            (6, 22),
...
def _write_decision_template(output_dir: Path, plan: dict[str, Any]) -> None:
    payload = {
        ...
        json.dumps(
            {
                ...
                "examples": sum(
                    len(row["review_ids"]) for row in manifest["records"]
                ),
