"""Game-held screening for label-free broadcast-clock state evidence."""
...
VARIANT_NAMES = ("base", "base+broadcast_raw")


def screen_shot_broadcast_fusion(
    *,
    ...
    """Compare base and broadcast-state variants under outer game isolation."""

    if pca_components < 1 or regularization_c <= 0:
        raise ValueError("broadcast fusion PCA and regularization must be positive")
    ...
    if broadcast["source_scene_artifact_sha256"] != scene["artifact_sha256"]:
        raise ValueError("broadcast state evidence does not bind the scene artifact")

    examples, base_matrix = _base_matrix(scene, videos)
    keys = [_example_key(row) for row in examples]
    evidence_lookup = {
        _example_key(row): row for row in broadcast["examples"]
    }
    if set(evidence_lookup) != set(keys) or len(evidence_lookup) != len(keys):
        raise ValueError("broadcast state evidence must exactly cover scene examples")
    broadcast_matrix = np.asarray(
        [evidence_lookup[key]["features"] for key in keys],
        dtype=np.float64,
    )
    target = np.asarray(
    ...
    if len(unique_groups) < 4:
        raise ValueError("broadcast fusion requires at least four games")
    ...
    for held_group in unique_groups:
        ...
        broadcast_scaler = StandardScaler().fit(
            broadcast_matrix[training_mask]
        )
        broadcast_training = broadcast_scaler.transform(
            broadcast_matrix[training_mask]
        )
        broadcast_held = broadcast_scaler.transform(
            broadcast_matrix[held_mask]
        )
        fold_inputs = {
            "base": (reduced_training, reduced_held),
            "base+broadcast_raw": (
                np.concatenate(
                    [reduced_training, broadcast_training], axis=1
                ),
    ...
    payload = {
        ...
        "purpose": "offline_label_free_broadcast_state_fusion_screening",
        "runtime_consumable": False,
        "codex_runtime_answer_used": False,
        "selection_protocol": "outer_game_held_broadcast_state_oof",
